...
void Session::Start()
{
	...
	boost::asio::async_read(_socket, boost::asio::buffer(rece_head_node->_data, HEAD_LENGTH),
		std::bind(&Session::HandleHeadRead, this, std::placeholders::_1, std::placeholders::_2, shared_from_this()));
}
...
void::Session::HandleMsgRead(const boost::system::error_code& error, size_t bytes_transfered, shared_ptr<Session> _self_shared)
{
	if (!error)
	{
		std::chrono::milliseconds dura(2000);
		std::this_thread::sleep_for(dura);
		cout << "receive msg length is : " << bytes_transfered << endl;
		cout << "receive msg is : ";
		cout.write(rece_msg_node->_data, bytes_transfered);
		cout << endl;
		send(rece_msg_node->_data, bytes_transfered);

		rece_head_node->Clear();
		boost::asio::async_read(_socket, boost::asio::buffer(rece_head_node->_data, HEAD_LENGTH),
			std::bind(&Session::HandleHeadRead, this, std::placeholders::_1, std::placeholders::_2, _self_shared));
	}
	else
	{
		cout << "HandleMsgRead failed error is : " << error.what() << endl;
		Close();
		_server->ClearSession(_uuid);
	}
}

void Session::HandleHeadRead(const boost::system::error_code& error, size_t bytes_transfered, shared_ptr<Session> _self_shared)
{
	if (!error)
	{
		short head_size;
		memcpy(&head_size, rece_head_node->_data, HEAD_LENGTH);

		if (head_size > MAX_LENGTH)
		{
			cout << "receive size is invaild length is :" << head_size << endl;
			Close();
			_server->ClearSession(_uuid);
			return;
		}

		rece_msg_node = make_shared<MsgNode>(head_size);
		boost::asio::async_read(_socket, boost::asio::buffer(rece_msg_node->_data, head_size),
			std::bind(&Session::HandleMsgRead, this, placeholders::_1, placeholders::_2, _self_shared));
	}
	else
	{
		cout << "HandleHeadRead failed error is : " << error.what() << endl;
		Close();
		_server->ClearSession(_uuid);
	}
}
	...
	if (!error)
	{
		...
		while (bytes_transfered > 0)
		{
			if (!b_head_parse)
			{
				...
				std::cout << "receive message is :" << rece_msg_node->_data << endl;
			...
			rece_msg_node->_data[rece_msg_node->total_len] = '\0';
			std::cout << "receice message is :" << rece_msg_node->_data << endl;
